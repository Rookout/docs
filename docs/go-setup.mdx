...
```go 
rookout.Start(rookout.RookOptions{token=[Your Rookout Token], labels=map[string]string{"env": "dev"}})
...
Rookout supports Go version 1.15 to 1.19 on Mac and Linux (Debian, Alpine, Photon, etc) operating systems.
...
Please avoid omitting them by setting the `-w` or `-s` [ldflags](https://pkg.go.dev/cmd/link). 
