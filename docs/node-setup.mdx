...
-   [**TypeScript**](https://www.typescriptlang.org/) - add either `"inlineSourceMap": true` or `"sourceMap": true` to the TypeScript config file, and also add `"inlineSources": true`. ([reference](https://www.typescriptlang.org/tsconfig#inlineSourceMap)).
