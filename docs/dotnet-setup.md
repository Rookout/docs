...
Disabling compiler optimizations `` will further improve the debugging experience at a small cost to the application performance.

### Multi-Project Solutions

To support multi-projects Solutions its recommended to add the following `` file to your Root folder:
...
To be able to debug libraries loaded using [``](https://docs.microsoft.com/en-us/dotnet/api/system.appdomain.load?view=netcore-3.1#System_AppDomain_Load_System_Byte___System_Byte___) make sure to load those binaries into Rookout using:
...
Rookout provides an easy to use wrapper - wrap your code with `` as in the example below:
