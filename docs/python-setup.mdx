...
For pre-forking servers please read [the relevant section](#pre-forking-celery-gunicorn-etc).
...
| `fork`                      | `ROOKOUT_ENABLE_FORK`                       | None         | Set to `True` to enable support in forked processes. Leave empty to auto detect                                                                     |
...
If you are using one of those application servers or frameworks, you could set the `fork` argument in the SDK API or the ROOKOUT_ENABLE_FORK environment variable to true to automatically enable Rookout in forked processes. Leaving the `fork` argument empty means that Rookout will auto detect if it should enable this feature.
