...
Update your `serverless.yaml` to have Rookout's environment variables configured for your project

```yaml
provider:
    ...
    environment:
      ROOKOUT_REMOTE_ORIGIN: git@github.com:Rookout/tutorial-nodejs-serverless-2022.git
      ROOKOUT_COMMIT: master

```
##### you can change `ROOKOUT_REMOTE_ORIGIN` to your project's remote origin and update the `ROOKOUT_COMMIT` to any commit in that poject
...
✔ Service deployed to stack tutorial-node-js-serverless-2022-dev (70s)
...
After a successful deployment we can access our function through [https://[bucket-name].s3.amazonaws.com/index.html](https://[bucket-name].s3.amazonaws.com/index.html)
